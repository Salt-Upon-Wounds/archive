...
describe("check update feature", () => {
  test("at the beginning, there is not any update button and there is 'all changes saved' phrase", async () => {
    renderWithAuthentication(
      wrapper(<EditQuestion examId={1} questionId={3} />)
    );
  ...
  test("user can change text description and update; loading message must be labeled on the update button when component is loading", async () => {
    renderWithAuthentication(
      wrapper(<EditQuestion examId={1} questionId={3} />)
    );
    await wait(100);
...
describe("check readonly property", () => {
  test("check all the textbox fields are readonly", async () => {
    renderWithAuthentication(
      wrapper(<EditQuestion readOnly={true} examId={1} questionId={3} />)
    );
    await wait(100);
